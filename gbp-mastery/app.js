(() => {
  const KEY = "osideGbpMastery.v1";
  const modules = window.GBP_MODULES || [];
  ...
  const labs = [
    {
      id: "eligibility-audit",
      title: "Eligibility and profile-safety gate",
      description: "Decide whether the business, location, and proposed profile are compliant.",
      task: "Classify the business as storefront, service-area, hybrid, or ineligible. Record signage, staffing, customer contact, address display, service areas, duplicate risk, categories, and the three conditions that would stop optimization.",
      proof: "Another reviewer can reach the same eligibility and address-display decision."
    },
    {
      id: "access-verification",
      title: "Ownership and verification plan",
      description: "Secure access and prepare authentic verification evidence.",
      task: "Create a client-owned access matrix, account inventory, verification-method response plan, one-take video shot list, evidence folder, re-verification triggers, and offboarding rules.",
      proof: "The business retains ownership and the evidence establishes real-world operation."
    },
    {
      id: "profile-blueprint",
      title: "Profile optimization blueprint",
      description: "Translate business reality into accurate, useful profile fields.",
      task: "Specify name, business type, address visibility, pin, phone, website, hours, primary/secondary categories, services, service areas, attributes, description, links, media, posts, and owners.",
      proof: "Every field is supported by evidence and a customer need."
    },
    {
      id: "review-system",
      title: "Review and reputation system",
      description: "Build compliant review acquisition and response operations.",
      task: "Create the request trigger, all-customer fairness rule, link/QR delivery, reminders, response SLA, negative-review playbook, reporting criteria, extortion protocol, and monthly metrics.",
      proof: "The system grows authentic reviews without gating or incentives."
    },
    {
      id: "content-proof",
      title: "Media and customer-proof plan",
      description: "Use authentic content to answer customer trust questions.",
      task: "Build a 30-photo shot list, four 30-second video concepts, 12 posts, permission/privacy rules, refresh cadence, description review, social/link plan, and assigned content owner.",
      proof: "Every asset shows real business evidence or answers a customer question."
    },
    {
      id: "rank-grid",
      title: "Local-rank grid diagnostic",
      description: "Summarize a 5×5 local-rank scan without losing geographic context.",
      task: "Enter 25 ranks from left-to-right, top-to-bottom. Use 21 for not found beyond rank 20. Interpret average, median, top-3 share, top-10 share, and not-found share alongside competitor locations and serviceability.",
      proof: "The written diagnosis separates observed pattern, likely cause, uncertainty, and next test.",
      calculator: "rank"
    },
    {
      id: "recovery-drill",
      title: "Profile recovery drill",
      description: "Prepare for duplicates, unauthorized edits, or suspension.",
      task: "Create a timeline, profile ID record, access audit, guideline comparison, corrective-action order, authentic evidence packet, appeal narrative, client communication, and a strict no-circumvention rule.",
      proof: "The case is concise, policy-specific, and supported by authentic documents."
    },
    {
      id: "capstone",
      title: "Capstone account audit",
      description: "Integrate the entire operating system.",
      task: "Audit a real redacted or hypothetical roofing profile. Produce eligibility and access findings, field QA, category/services plan, review/media systems, website/prominence analysis, competitor matrix, rank grids, performance-to-job reporting, risk notes, top-three priorities, and a client walkthrough.",
  ...
  function renderDashboard() {
    ...
    app.innerHTML = `
      <section class="hero gbp-hero">
        <div class="hero-copy">
          <p class="eyebrow">Oside Digital · Local visibility</p>
          <h1>Master Google Business Profile as a trust and local-discovery system—not a field-stuffing exercise.</h1>
          <p class="lede">Learn eligibility, verification, profile optimization, reviews, local ranking, grids, measurement, recovery, and roofing-client strategy.</p>
        ...
        <aside class="hero-panel">
          ...
          <p class="muted">Source foundation reviewed June 21, 2026. The full baseline is optional for beginners; Module 1 is the recommended start.</p>
        </aside>
      </section>
      ...
      <section class="report-section">
        ...
        <div class="report-grid">
          <article class="report-card"><h3>12 modules</h3><p class="muted">Current GBP guidance translated into roofing decisions and fieldwork.</p></article>
          ...
          <article class="report-card"><h3>8 applied labs</h3><p class="muted">Eligibility, verification, reviews, content, rank grids, recovery, and capstone.</p></article>
        </div>
      </section>
  ...
  function calculatorMarkup(type) {
    if (type === "rank") {
      return `
        <div class="calculator rank-calculator">
          <label class="rank-input-label">25 grid ranks, separated by commas
            <textarea class="rank-values" placeholder="2,3,4,6,9,3,2,3,7,12,4,3,2,8,14,7,6,5,11,18,12,10,9,17,21">2,3,4,6,9,3,2,3,7,12,4,3,2,8,14,7,6,5,11,18,12,10,9,17,21</textarea>
          </label>
          <div class="rank-metrics">
            <div><span>Average</span><strong class="rank-average">—</strong></div>
            <div><span>Median</span><strong class="rank-median">—</strong></div>
            <div><span>Top 3</span><strong class="rank-top3">—</strong></div>
            <div><span>Top 10</span><strong class="rank-top10">—</strong></div>
            <div><span>Not found</span><strong class="rank-missing">—</strong></div>
          </div>
          <p class="rank-warning" role="status"></p>
        </div>
      `;
    }
    return `
  ...
  function bindLab(lab) {
    ...
    if (lab.calculator) {
      const update = () => {
        if (lab.calculator === "rank") {
          const values = card.querySelector(".rank-values").value
            .split(/[\s,]+/)
            .filter(Boolean)
            .map(Number)
            .filter(Number.isFinite);
          const warning = card.querySelector(".rank-warning");
          if (values.length !== 25) {
            warning.textContent = `Enter exactly 25 ranks. Current count: ${values.length}.`;
            [".rank-average", ".rank-median", ".rank-top3", ".rank-top10", ".rank-missing"].forEach(selector => card.querySelector(selector).textContent = "—");
            return;
          }
          warning.textContent = "Use the raw grid and context with these summary metrics.";
          const sorted = [...values].sort((a, b) => a - b);
          const average = values.reduce((sum, value) => sum + value, 0) / values.length;
          const percent = threshold => Math.round((values.filter(value => value <= threshold).length / values.length) * 100);
          const missing = Math.round((values.filter(value => value > 20).length / values.length) * 100);
          card.querySelector(".rank-average").textContent = average.toFixed(1);
          card.querySelector(".rank-median").textContent = sorted[12];
      ...
      card.querySelectorAll(".calculator textarea").forEach(input => input.addEventListener("input", update));
      update();
    }
  }
  ...
  function reportData() {
    ...
    return {
      ...
      results,
      weakest,
      setup: readiness(["fundamentals", "eligibility", "ownership-verification", "core-information"]),
      operate: readiness(["core-information", "profile-optimization", "reviews", "content-media", "prominence", "rank-diagnostics", "measurement"]),
      advise: readiness(["fundamentals", "eligibility", "rank-diagnostics", "measurement", "recovery", "roofing-capstone"])
    };
  }
  ...
  function renderReport() {
    ...
    app.innerHTML = `
      <header class="report-header">
        <p class="eyebrow">Google Business Profile mastery report</p>
      ...
      <section class="report-grid">
        ${readinessCard("Ready to set up profiles?", data.setup)}
        ${readinessCard("Ready to optimize profiles?", data.operate)}
  ...
  function thirtyDayPlan(weakest) {
    ...
    return `
      ...
      <div class="plan-week"><h3>Days 15–21 · Integrate</h3><p>Study <strong>${esc(names[2])}</strong> and <strong>${esc(names[3])}</strong>. Complete a competitor matrix, rank-grid diagnosis, and one decision log.</p></div>
  ...
  function exportReport(data) {
    let md = `# Oside Google Business Profile Mastery Report\n\nExported ${new Date().toLocaleString()}\n\n`;
    ...
    md += `| Ready to set up profiles? | ${data.setup.status} | ${data.setup.average}% |\n`;
    md += `| Ready to optimize profiles? | ${data.operate.status} | ${data.operate.average}% |\n`;
    ...
    md += "\n## Official-source reminder\n\nRecheck current Google Business Profile documentation before changing a live profile: https://support.google.com/business/?hl=en\n";
    const blob = new Blob([md], { type: "text/markdown;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = `oside-gbp-mastery-${new Date().toISOString().slice(0, 10)}.md`;
  ...
  function reset() {
    if (!window.confirm("Clear all GBP learning, lab, and diagnostic progress?")) return;
